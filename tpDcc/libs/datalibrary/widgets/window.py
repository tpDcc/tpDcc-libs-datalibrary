...
from tpDcc.libs.qt.core import qtutils, base, menu, animation, decorators as qt_decorators
...
from tpDcc.libs.datalibrary.core import consts, factory
from tpDcc.libs.datalibrary.widgets import viewer, search, sidebar, status
from tpDcc.libs.datalibrary.widgets.menus import filter, group, sort, libraries
...
class LibraryWindow(base.BaseWidget):
    ...
    def __init__(self, dcc_client, name='', items_factory=None, json_settings_file_path=None, parent=None):

        self._dcc_client = dcc_client
        ...
        self._items_factory = items_factory or factory.ItemsFactory()
        ...
        self.set_library(self.LIBRARY_CLASS(library_window=self))
        ...
        self.set_path(r'D:\rigscript_projects\test2')
    ...
    def update_sidebar(self):
        ...
        for item in items:
            current_data[item.path] = item.data
    ...
    def create_folder_context_menu(self):
        ...
        if path:
            ...
            items = self.library().find_items_views(queries)
            if items:
                self._items = items
    ...
    def _create_new_item_menu(self):
        ...
        for data_item_class in sorted(data.get_all_data_items(), key=operator.attrgetter('MENU_ORDER')):
            view_item_class = self.factory.get_view_class_from_item_class(data_item_class)
            action = view_item_class.create_action(data_item_class, menu, self)
            if action:
