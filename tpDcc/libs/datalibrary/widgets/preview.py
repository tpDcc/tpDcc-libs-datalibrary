...
from tpDcc.libs.datalibrary.widgets import sequence
...
class PreviewWidget(base.BaseWidget, object):
    ...
    def ui(self):
        ...
        schema = self._item.load_schema() if self._item else None
        if schema:
            self._form_widget = formwidget.FormWidget(self)
            self._form_widget.setObjectName('{}Form'.format(self._item.__class__.__name__))
    ...
    def setup_signals(self):
        self._item.dataChanged.connect(self._on_item_data_changed)
    ...
    def validator(self, **kwargs):
        ...
        self._item.load_validator(**kwargs)
        self.update_icon()
    ...
    def update_icon(self):
        ...
        # Updates static icon
        icon = self._item.thumbnail_icon()
        if icon:
            self._sequence_widget.setIcon(icon)

        # Update images sequence (if exist)
        if self._item.image_sequence_path():
            self._sequence_widget.set_dirname(self._item.image_sequence_path())
