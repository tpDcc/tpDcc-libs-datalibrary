...
def default_library(settings_file_path=None):
    """
    Returns default library name
    :return: str
    """

    libraries = read_settings(settings_file_path)

    for name in libraries:
        if libraries[name].get('default'):
            return name

    if consts.DEFAULT_LIBRARY_NAME in libraries:
        return consts.DEFAULT_LIBRARY_NAME
...
def add_library(name, path, **settings):
    """
    Adds a new library with the given name, path and settings
    :param name: str
    :param path: str
    :param settings: dict
    """

    libraries = read_settings(settings.pop('settings_file_path', None))
    libraries.setdefault(name, dict())
    libraries[name]['path'] = path
    update(libraries[name], settings)
    save_settings(libraries)


def remove_library(name, settings_file_path=None):
    """
    Removes a library by name
    :param name: str
...
def create_temp_path(name, clean=True, makedirs=True):
    """
